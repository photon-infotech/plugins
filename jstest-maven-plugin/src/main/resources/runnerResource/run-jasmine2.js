...
if (phantom.args.length === 0) {
    ...
    var setupPageFn = function(p, k) {
        return function() {
            ...
            setupWriteFileFunction(p, k, fs.separator, args[1]);
        };
    };
...
function setupWriteFileFunction(page, key, path_separator, reportDir) {
    page.evaluate(function(){
        window["%resultsObj%"] = {};
        window.fs_path_separator = "%fs_path_separator%";
        window.phrescoReportDir = "%phrescoReportDir%";
        window.__phantom_writeFile = function(filename, text) {
            window["%resultsObj%"][filename] = text;
        };
    }, {resultsObj: key, fs_path_separator: path_separator.replace("\\", "\\\\"), phrescoReportDir: reportDir});
}
