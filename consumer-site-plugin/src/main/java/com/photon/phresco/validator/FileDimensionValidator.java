...
public class FileDimensionValidator extends BaseValidator {
		...
		for (Element validation : validationList) {
				...
				if (fileTypeValue.equals(IMAGE)) {
					...
					if (expectedHeightString != null
						...
						validationStatusList
								.addAll(checkImageHeight(File.separator
										+ fileNameValue, expectedHeight));
					}
					...
					if (expectedWidthString != null
						...
						validationStatusList.addAll(checkImageWidth(
								File.separator + fileNameValue, expectedWidth));
					}
				}
			}
			for (Element folder : folderList) {
				...
				for (Element fileOne : fileListOne) {
					...
					if (fileTypeValue.equals(IMAGE)) {
						String expectedWidthString = fileOne.getChild(WIDTH)
								.getValue();
						String expectedHeightString = fileOne.getChild(HEIGHT)
								.getValue();
						if (expectedHeightString != null
								&& expectedHeightString.length() > 0) {
							int expectedHeight = Integer
				...
				for (Element folderOne : folderListOne) {
					...
					for (Element fileTwo : fileListTwo) {
						...
						if (fileTypeValue.equals(IMAGE)) {
							String expectedWidthString = fileTwo
							...
							if (expectedHeightString != null
								...
								int expectedHeight = Integer
										.parseInt(expectedHeightString);
								validationStatusList.addAll(checkImageHeight(
	...
	private List<ValidationStatus> checkImageWidth(String filename,
			int expectedWidth) throws MojoExecutionException, IOException {
		List<ValidationStatus> validationStatusList = new ArrayList<ValidationStatus>();
		if (filename.indexOf('!') == -1) {
			...
			if (width != expectedWidth) {
				...
				validationStatusList.add(new ValidationStatus(true, filename
						+ " has exactly the correct width"));
			}
			return validationStatusList;
		} else {
			for (String localDirectory : localDirectories) {
				...
				if (width != expectedWidth) {
					...
					validationStatusList.add(new ValidationStatus(true,
							filename + " has exactly the correct width"));
				}
			}
		}
		return validationStatusList;
	...
	private List<ValidationStatus> checkImageHeight(String filename,
			int expectedHeight) throws MojoExecutionException, IOException {
		List<ValidationStatus> validationStatusList = new ArrayList<ValidationStatus>();
		if (filename.indexOf('!') == -1) {
			BufferedImage bimg = ImageIO.read(new File(loc + filename));
			...
			for (String localDirectory : localDirectories) {
				...
				replacementFileName = filename.replace("!languages",
						localDirectory);
				BufferedImage bimg = ImageIO.read(new File(loc
						+ replacementFileName));
				int height = bimg.getHeight();
				if (height != expectedHeight) {
					validationStatusList
