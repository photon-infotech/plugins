...
public class FileValidator extends BaseValidator {
	...
	public List<ValidationStatus> validate() throws MojoExecutionException,
		...
		for (Element validation : validationList) {
			...
			for (Element file : fileList) { // five nested levels
				...
				validationStatusList.addAll(checkIfFileExists(File.separator
						+ fileNameValue, fileRequiredValue));
			}
			for (Element folder : folderList) {
				...
				for (Element fileOne : fileListOne) {
					...
					validationStatusList.addAll(checkIfFileExists(
	...
	private List<ValidationStatus> checkIfFileExists(String stringFile,
			...
			for (String localDirectory : localDirectories) {
				...
				if (new Boolean(required)) {
					if (file.exists()) {
						...
						System.out.println(replacementStringFile
								+ " does not Exist");
						validationStatusList.add(new ValidationStatus(false,
