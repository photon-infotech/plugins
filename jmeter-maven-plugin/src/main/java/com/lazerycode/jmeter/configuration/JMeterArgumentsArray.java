...
public class JMeterArgumentsArray {
	...
	public void setTestFile(File value) {
		...
		if(resultFilesName == null){
			...
			resultsLogFileName = resultsDirectory + File.separator + resultFilesName + resultFileExtension;
		}
		argumentList.add(TESTFILE_OPT);
		argumentList.add(LOGFILE_OPT);
	}
	
	public String testResultFile(File value) {
		testFile = value.getAbsolutePath();
		if(resultFilesName == null){
			 resultsName =  value.getName().substring(0, value.getName().lastIndexOf(".")) + resultFileExtension;
		}
		else {
			 resultsName =  resultFilesName + resultFileExtension;
		}
		return resultsName;
	}
