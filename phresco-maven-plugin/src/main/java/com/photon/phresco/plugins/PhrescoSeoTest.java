 ...
 * 
 */
public class PhrescoSeoTest extends PhrescoAbstractMojo {
	/**
     * The Maven project.
     * 
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		try {
        	...
        	File infoFile = new File(baseDir + File.separator +  Constants.DOT_PHRESCO_FOLDER  + File.separator +  Constants.SEO_INFO_FILE);
    		if (StringUtils.isNotEmpty(moduleName)) {
        		infoFile = new File(baseDir + File.separator + moduleName + File.separator + Constants.SEO_INFO_FILE);
        	} 
    		PhrescoPlugin plugin = getPlugin(getDependency(infoFile.getPath(), Constants.PHASE_SEO_TEST), mavenSession, project, container);
    		MojoProcessor processor = new MojoProcessor(infoFile);
        	Configuration configuration = processor.getConfiguration(Constants.PHASE_SEO_TEST);
        	plugin.seoTest(configuration, getMavenProjectInfo(project, moduleName));
